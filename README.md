...
This is a webcam created by Samsung Internet to showcase the device posture API and viewport segment API on a Samsung Galaxy Flip using WebRTC.
...
this allows web applications to request and be notified of changes of the posture of a device. But that’s not the only thing that we’ve working on, after listening to the developer’s community feedback we decided to
keep integrating new features to support the integration of foldable devices.We also implemented the viewport segment API in our browser. The viewport segment api and the device posture api can
work together to take maximum advantage of foldables.
Web Apps can take advantage of the new form factors and improve user's experience.
...
### Using the Device Posture API and Viewport Segment API
...
The viewport segments api, in the other hand, is an experimental media feature designed to detect if your website is being displayed on a dual-
screen device. The proposed name of the media feature is viewport-segments. There are two varieties: horizontal-viewport-
segments and vertical-viewport-segments, we can use this media querys combined with the device posture api to also check
if the device is currently having a horizontal o vertical orientation within its hinge, which can be really helpful for Samsung
devices.

Let’s say that we have a Galaxy Z Flip, we will be interested in changing the layout when the hinge orientation is horizontal,
but, in the other hand, if we have a Galaxy Z Fold instead it will be when the device's hinge has a vertical orientation.
The viewport segment api can help us differentiate this using the media queries horizontal-viewport-segment when we have a vertical hinge (Galaxy Fold)
and vertical-viewport-segments for a horizontal one (Galaxy Z Flip).

![alt text](https://cdn.glitch.global/8375261e-ec40-472b-ac98-93dee6accb41/viewport.jpg?v=1661514719889 "Viewport Segment API")
...
Using modern css features like display:flex, we can change the layout of the body element and the elements with the class video-container and
camera-controls when the phone is flipped and has a horizontal hinge.
